// One-off admin endpoint: pushes the Google Sheet catalog into Shopify.
// This is the serverless twin of scripts/sync-shopify.mjs -- it exists
...
// Serverless functions have a short execution ceiling, and syncing the
// whole Sheet in one call (Shopify round-trip per row, throttled) can
// easily blow past it. So this runs as small, repeatable batches instead
// of one long request: ?mode=sync&offset=0&limit=10 does one slice and
// reports where the next one starts; ?mode=archive is called once at the
// very end, after every row has been synced, and re-derives the full set
// of still-published SKUs itself (rather than needing state carried across
// the earlier stateless batch calls) to archive anything no longer in the Sheet.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (mode === 'archive') {
      const syncedSkus = new Set(rows.map((row) => row.sku));
      const archived = await archiveMissingProducts(token, syncedSkus);
      res.statusCode = 200;
      res.end(JSON.stringify({ sheetRows: rows.length, archived }, null, 2));
      return;
    }

    const offset = Number.parseInt(req.query.offset, 10) || 0;
    const limit = Number.parseInt(req.query.limit, 10) || BATCH_LIMIT;
    const batch = rows.slice(offset, offset + limit);

    const locationId = batch.length ? await getPrimaryLocationId(token) : null;
