 ...
 * @param {string} piece.uploadUrl     Printed on the certificate, no scheme
 * @param {string} [piece.uploadQrUrl] Encoded in the QR; defaults to uploadUrl
...
async function buildCertificateHtml(piece) {
  ...
  // The QR and the printed line are allowed to differ, and usually should.
  // A QR can carry the full signed token, which names the adopter and gives
  // the personalised page; the printed line has to stay short enough to read
  // and type, so it carries the short code. Both resolve to the same piece --
  // what must never happen is one of them not working, which is exactly what
  // shipped on the first real certificate.
  const qr = await QRCode.toDataURL(`https://${piece.uploadQrUrl || piece.uploadUrl}`, {
    // A QR needs a quiet zone around it or decoders can fail to find the
    // symbol at all. margin:0 removed it -- phone cameras coped, a generic
    // decoder did not. Keep this non-zero.
    margin: 2,
    width: 240,
